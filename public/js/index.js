...
let a = getData()
  .then((data) => {
    ...
    dataObj.forEach((element, i) => {
      html += `<tr>
                        <th scope="row"><div class= "form-check"> <input type="radio" onclick="delById()" name="radio" class="select" id="${
                          i + 1
                        }" value = "${element._id}"></th></div>
                        <th scope="row">${i + 1}</th>
                        <td>${element.name}</td>
      ...
      html2 += `<div class="col mb-4" >
        ...
        element.name
                                    ...
                                    <a href="/${
                                      element._id
                                    }/update" class="btn btn-info">Update</a>
                                    <a href="/${
                                      element._id
                                    }/mail" class="btn btn-success">Send Mail</a>
                                </div>
                            </div>
                        </div>`;
